...
def flatc(options, schema, prefix=None, include=None, data=None, cwd=tests_path):
    ...
    cmd = [str(flatc_path)] + options + ["--preserve-case"]
    if prefix:
        cmd += ["-o"] + [prefix]
    if include:
        cmd += ["-I"] + [include]
...
flatc(
    options=["--ts", "--reflect-names", "--gen-name-strings", "--gen-mutable",
             "--gen-object-api", "--ts-entry-points", "--ts-flat-files"],
...
esbuild("union_vector/union_vector.ts",
...
flatc(
    options=["--ts", "--reflect-names",
             "--gen-name-strings", "--ts-no-import-ext"],
    schema="../optional_scalars.fbs",
