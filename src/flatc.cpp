...
const static FlatCOption flatc_options[] = {
    ...
    {"", "schema-in", "FILE",
     "Import a JSON Schema previously generated by --jsonschema and treat it "
     "as the input schema."},
    {"", "bfbs-filenames", "PATH",
  ...
  for (int argi = 1; argi < argc; argi++) {
    ...
    if (arg[0] == '-') {
      ...
      if (arg == "-o") {
        ...
        options.schema_filenames.push_back(path);
        options.filenames.push_back(path);
      } else if (arg.rfind("--schema-in=", 0) == 0) {
        std::string path =
            flatbuffers::PosixPath(arg.substr(std::string("--schema-in=").size()));
        options.schema_filenames.push_back(path);
        options.filenames.push_back(path);
      } else if (arg == "-M") {
...
std::unique_ptr<Parser> FlatCompiler::GenerateCode(const FlatCOptions& options,
  ...
  std::unordered_set<std::string> schema_inputs(
      options.schema_filenames.begin(), options.schema_filenames.end());
  ...
  for (auto file_it = options.filenames.begin();
    ...
    const bool is_json_schema = schema_inputs.count(filename) > 0;
    ...
    if (is_json_schema &&
        flatbuffers::GetExtension(filebase) == "schema") {
