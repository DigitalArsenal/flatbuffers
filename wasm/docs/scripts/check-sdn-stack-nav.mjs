...
const distCopyCommands = workflow.match(/^\s*cp\b[^\n]*wasm\/docs\/dist[^\n]*$/gmu) ?? [];
if (distCopyCommands.length !== 1 || distCopyCommands[0].trim() !== "cp -a wasm/docs/dist/. docs/") {
  throw new Error(`${workflowPath} must copy the complete reviewed dist tree to docs/ exactly once`);
}
assertContains(workflow, "find docs -mindepth 1 -maxdepth 1", workflowPath);
if (workflow.indexOf("find docs -mindepth 1 -maxdepth 1") > workflow.indexOf("cp -a wasm/docs/dist/. docs/")) {
  throw new Error(`${workflowPath} must clear generated docs output before copying dist/`);
}
