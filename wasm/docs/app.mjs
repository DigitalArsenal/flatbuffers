...
function generateVCard(info, { skipPhoto = false } = {}) {
  ...
  // Photo (data URI) - skip for QR code since it exceeds QR capacity
  ...
  vcard = vcard.replace(
    ...
    (_, type, b64) => {
      ...
      // vCard 3.0 line folding: first line is the property, continuation lines start with a space
...
function setupMainAppHandlers() {
  ...
  // Helper: reset photo preview to placeholder
  ...
  // Helper: show captured/uploaded photo in preview
  function showPhotoPreview(dataUrl) {
    const preview = $('vcard-photo-preview');
    if (!preview) return;
    // Hide placeholder and video
    const placeholder = preview.querySelector('.photo-placeholder-icon');
  ...
  // Camera support: show "Take Photo" if getUserMedia is available
  ...
  if (navigator.mediaDevices && navigator.mediaDevices.getUserMedia) {
    ...
    cameraBtn?.addEventListener('click', async () => {
      try {
        ...
        // Hide placeholder and any existing img
        const preview = $('vcard-photo-preview');
        if (preview) {
          const placeholder = preview.querySelector('.photo-placeholder-icon');
          if (placeholder) placeholder.style.display = 'none';
  ...
  if ('contacts' in navigator && 'ContactsManager' in window) {
    const contactsBtn = $('vcf-contacts-btn');
    ...
    contactsBtn?.addEventListener('click', async () => {
      try {
        ...
        const available = props.filter(p => supported.includes(p));
        const contacts = await navigator.contacts.select(available, { multiple: false });
        if (contacts.length) {
          const c = contacts[0];
          if (c.name && c.name[0]) {
            const parts = c.name[0].split(' ');
            const firstEl = $('vcard-firstname');
            const lastEl = $('vcard-lastname');
            if (firstEl) firstEl.value = parts[0] || '';
            if (lastEl) lastEl.value = parts.slice(1).join(' ') || '';
          }
          if (c.email && c.email[0]) {
            const emailEl = $('vcard-email');
            if (emailEl) emailEl.value = c.email[0];
          }
          if (c.icon && c.icon[0]) {
            // icon is a Blob - resize to 128x128
            const blob = c.icon[0];
  ...
  $('generate-vcard')?.addEventListener('click', async () => {
    ...
    try {
      ...
      $('vcard-result-view').style.display = '';
    } catch (err) {
  ...
  $('vcard-back-btn')?.addEventListener('click', () => {
    $('vcard-result-view').style.display = 'none';
