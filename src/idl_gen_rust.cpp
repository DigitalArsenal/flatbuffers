...
class RustGenerator : public BaseGenerator {
 ...
 private:
  ...
  void GenTable(const StructDef& struct_def) {
    ...
    // Vec<u8> doesn't implement Copy, so encrypted accessors only derive Clone.
    if (has_encrypted) {
  ...
  void GenStruct(const StructDef& struct_def) {
    ...
    ForAllStructFields(struct_def, [&](const FieldDef& field) {
      ...
      if (IsStruct(field.value.type)) {
        ...
        if (field.attributes.Lookup("encrypted") != nullptr) {
          code_ +=
              "    let raw_value = ::flatbuffers::EndianScalar::from_little_endian(unsafe {";
        } else {
          code_ +=
        ...
        if (field.attributes.Lookup("encrypted") != nullptr) {
          code_ +=
              "    crate::flatbuffers_encryption::decrypt_scalar(raw_value, &self.encryption_ctx, {{FIELD_OFFSET}})";
        }
      }
      code_ += "}\n";
      // Setter.
