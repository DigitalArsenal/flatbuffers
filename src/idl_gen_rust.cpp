...
class RustGenerator : public BaseGenerator {
 public:
  ...
  // Generate the flatbuffers_encryption module for Rust
  bool GenerateEncryptionModule(const Namespace& ns) const {
    ...
    code += "//! Requires the `aes` and `ctr` crates.\n\n";
    code += "use aes::Aes256;\n";
    code += "use aes::cipher::{KeyIvInit, StreamCipher};\n";
    code += "use ctr::Ctr128BE;\n\n";
    code += "type Aes256Ctr = Ctr128BE<Aes256>;\n\n";

    // DeriveNonce helper
    code += "fn derive_nonce(ctx: &[u8], field_offset: u16) -> [u8; 16] {\n";
    code += "    let mut nonce = [0u8; 16];\n";
    code += "    nonce[..12].copy_from_slice(&ctx[..12]);\n";
    code += "    nonce[12..16].copy_from_slice(&(field_offset as u32).to_le_bytes());\n";
    code += "    nonce\n";
    code += "}\n\n";

    // DecryptBytes helper
    code += "fn decrypt_bytes(data: &mut [u8], ctx: &[u8], field_offset: u16) {\n";
    code += "    if ctx.len() < 32 {\n";
    code += "        panic!(\"Encryption context must be at least 32 bytes\");\n";
    code += "    }\n";
    code += "    let key: [u8; 32] = ctx[..32].try_into().unwrap();\n";
    code += "    let nonce = derive_nonce(ctx, field_offset);\n";
    code += "    let mut cipher = Aes256Ctr::new(&key.into(), &nonce.into());\n";
    code += "    cipher.apply_keystream(data);\n";
    code += "}\n\n";

    // decrypt_scalar generic function
    code += "pub fn decrypt_scalar<T: Copy>(value: T, ctx: &Option<Vec<u8>>, field_offset: u16) -> T\n";
    code += "where\n";
    code += "    T: bytemuck::Pod,\n";
    code += "{\n";
    code += "    match ctx {\n";
    code += "        None => value,\n";
    code += "        Some(ctx) => {\n";
    code += "            let mut bytes = bytemuck::bytes_of(&value).to_vec();\n";
    code += "            decrypt_bytes(&mut bytes, ctx, field_offset);\n";
    code += "            *bytemuck::from_bytes(&bytes)\n";
    code += "        }\n";
    code += "    }\n";
    code += "}\n\n";

    // decrypt_string function
    code += "pub fn decrypt_string(value: Option<&str>, ctx: &Option<Vec<u8>>, field_offset: u16) -> Option<String> {\n";
    code += "    match (value, ctx) {\n";
    code += "        (None, _) => None,\n";
    code += "        (Some(s), None) => Some(s.to_string()),\n";
    code += "        (Some(s), Some(ctx)) => {\n";
    code += "            let mut bytes = s.as_bytes().to_vec();\n";
    code += "            decrypt_bytes(&mut bytes, ctx, field_offset);\n";
    ...
    std::string filename = directories + "flatbuffers_encryption.rs";
  ...
  bool GenerateIndividualFiles() {
    ...
    bool result = GenerateSymbols<EnumDef>(
           ...
           GenerateSymbols<StructDef>(
               parser_.structs_, [&](const StructDef& s) {
                 ...
                 // Track namespaces with encrypted fields
                 if (HasEncryptedFields(s)) {
                   std::string ns_key = namer_.Directories(s.defined_namespace->components);
    ...
    // Generate encryption modules for namespaces that need them
    for (const auto& ns_key : encryption_namespaces) {
      // Find the namespace from any struct in this namespace
      for (auto it = parser_.structs_.vec.begin();
           it != parser_.structs_.vec.end(); ++it) {
        const StructDef& s = **it;
        if (namer_.Directories(s.defined_namespace->components) == ns_key) {
          if (!GenerateEncryptionModule(*s.defined_namespace)) {
            return false;
          }
          break;
        }
  ...
  bool GenerateOneFile() {
    ...
    if (needs_encryption) {
      ...
      code_ += "  ];";
      code_ += "  const RCON: [u8; 10] = [0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1b,0x36];";
      code_ += "";
      code_ += "  fn xtime(x: u8) -> u8 { ((x << 1) ^ (if x & 0x80 != 0 { 0x1b } else { 0 })) }";
      code_ += "";
      code_ += "  fn expand_key(key: &[u8]) -> [u8; 240] {";
      code_ += "    let mut expanded = [0u8; 240];";
      code_ += "    expanded[..32].copy_from_slice(&key[..32]);";
      code_ += "    let mut rcon_idx = 0usize;";
      code_ += "    let mut i = 32;";
      code_ += "    while i < 240 {";
      code_ += "      let mut t = [expanded[i-4], expanded[i-3], expanded[i-2], expanded[i-1]];";
      code_ += "      if i % 32 == 0 {";
      code_ += "        t = [SBOX[t[1] as usize] ^ RCON[rcon_idx], SBOX[t[2] as usize], SBOX[t[3] as usize], SBOX[t[0] as usize]];";
      code_ += "        rcon_idx += 1;";
      code_ += "      } else if i % 32 == 16 {";
      code_ += "        t = [SBOX[t[0] as usize], SBOX[t[1] as usize], SBOX[t[2] as usize], SBOX[t[3] as usize]];";
      code_ += "      }";
      ...
      code_ += "  }";
      code_ += "";
      code_ += "  fn aes_encrypt_block(block: &[u8; 16], expanded_key: &[u8; 240]) -> [u8; 16] {";
      code_ += "    let mut state = *block;";
      code_ += "    for i in 0..16 { state[i] ^= expanded_key[i]; }";
      code_ += "    for round in 1..=14 {";
      code_ += "      for i in 0..16 { state[i] = SBOX[state[i] as usize]; }";
      ...
      code_ += "        for c in 0..4 {";
      code_ += "          let i = c * 4;";
      code_ += "          let (s0, s1, s2, s3) = (state[i], state[i+1], state[i+2], state[i+3]);";
      code_ += "          let x = s0 ^ s1 ^ s2 ^ s3;";
      code_ += "          state[i] ^= x ^ xtime(s0 ^ s1);";
      code_ += "          state[i+1] ^= x ^ xtime(s1 ^ s2);";
      code_ += "          state[i+2] ^= x ^ xtime(s2 ^ s3);";
      code_ += "          state[i+3] ^= x ^ xtime(s3 ^ s0);";
      code_ += "        }";
      code_ += "      }";
      code_ += "      let offset = round * 16;";
      code_ += "      for i in 0..16 { state[i] ^= expanded_key[offset + i]; }";
      code_ += "    }";
      code_ += "    state";
      code_ += "  }";
      code_ += "";
      code_ += "  fn increment_counter(counter: &mut [u8; 16]) {";
      code_ += "    for i in (0..16).rev() {";
      code_ += "      counter[i] = counter[i].wrapping_add(1);";
      code_ += "      if counter[i] != 0 { break; }";
      code_ += "    }";
      code_ += "  }";
      code_ += "";
      code_ += "  fn derive_nonce(ctx: &[u8], field_offset: u16) -> [u8; 16] {";
      code_ += "    let mut nonce = [0u8; 16];";
      code_ += "    nonce[..12].copy_from_slice(&ctx[..12]);";
      code_ += "    nonce[12..16].copy_from_slice(&(field_offset as u32).to_le_bytes());";
      code_ += "    nonce";
      code_ += "  }";
      code_ += "";
      code_ += "  fn decrypt_bytes(data: &[u8], ctx: &[u8], field_offset: u16) -> Vec<u8> {";
      code_ += "    let key: [u8; 32] = ctx[..32].try_into().unwrap();";
      code_ += "    let mut counter = derive_nonce(ctx, field_offset);";
      code_ += "    let expanded_key = expand_key(&key);";
      code_ += "    let mut result = vec![0u8; data.len()];";
      code_ += "    let mut i = 0;";
      code_ += "    while i < data.len() {";
      code_ += "      let keystream = aes_encrypt_block(&counter, &expanded_key);";
      code_ += "      let block_len = core::cmp::min(16, data.len() - i);";
      code_ += "      for j in 0..block_len { result[i + j] = data[i + j] ^ keystream[j]; }";
      code_ += "      increment_counter(&mut counter);";
      code_ += "      i += 16;";
      code_ += "    }";
      code_ += "    result";
      code_ += "  }";
      code_ += "";
      code_ += "  pub fn decrypt_scalar<T: Copy + Sized>(value: T, ctx: &Option<Vec<u8>>, field_offset: u16) -> T {";
      code_ += "    match ctx {";
      code_ += "      None => value,";
      code_ += "      Some(c) => {";
      code_ += "        if c.len() < 32 { return value; }";
      code_ += "        let size = core::mem::size_of::<T>();";
      code_ += "        let bytes: &[u8] = unsafe { core::slice::from_raw_parts(&value as *const T as *const u8, size) };";
      code_ += "        let decrypted = decrypt_bytes(bytes, c, field_offset);";
      code_ += "        let mut result = core::mem::MaybeUninit::<T>::uninit();";
      code_ += "        unsafe {";
      code_ += "          core::ptr::copy_nonoverlapping(decrypted.as_ptr(), result.as_mut_ptr() as *mut u8, size);";
      code_ += "          result.assume_init()";
      code_ += "        }";
      code_ += "      }";
      code_ += "    }";
      code_ += "  }";
      code_ += "";
      code_ += "  pub fn decrypt_string(value: Option<&str>, ctx: &Option<Vec<u8>>, field_offset: u16) -> Option<String> {";
      code_ += "    match (value, ctx) {";
      code_ += "      (None, _) => None,";
      code_ += "      (Some(s), None) => Some(s.to_string()),";
      code_ += "      (Some(s), Some(c)) => {";
      code_ += "        if c.len() < 32 { return Some(s.to_string()); }";
      code_ += "        let decrypted = decrypt_bytes(s.as_bytes(), c, field_offset);";
      code_ += "        String::from_utf8(decrypted).ok()";
      code_ += "      }";
      code_ += "    }";
      code_ += "  }";
      code_ += "}";
 ...
 private:
  ...
  std::string GenTableAccessorFuncReturnType(const FieldDef& field,
    ...
    switch (GetFullType(field.value.type)) {
      ...
      case ftString: {
        // Encrypted strings return Option<String> since decryption creates a new string
        if (field.attributes.Lookup("encrypted") != nullptr) {
          return WrapOption("String");
        }
  ...
  std::string GenTableAccessorFuncBody(const FieldDef& field,
    ...
    if (field.attributes.Lookup("encrypted") != nullptr) {
      const Type& type = field.value.type;
      if (IsString(type)) {
        result = "crate::flatbuffers_encryption::decrypt_string(" + result +
                 ", &self.encryption_ctx, {{STRUCT_TY}}::" + vt_offset + ")";
      } else if (IsScalar(type.base_type)) {
        result = "crate::flatbuffers_encryption::decrypt_scalar(" + result +
                 ", &self.encryption_ctx, {{STRUCT_TY}}::" + vt_offset + ")";
      }
    }
  ...
  void GenTable(const StructDef& struct_def) {
    ...
    // Vec<u8> doesn't implement Copy, so we can't derive Copy for structs with encryption_ctx
    if (has_encrypted) {
      code_ += "#[derive(Clone, PartialEq)]";
    } else {
    ...
    if (has_encrypted) {
      code_ += "  pub encryption_ctx: Option<Vec<u8>>,";
    }
    code_ += "}";
    code_ += "";
    ...
    code_ += "  unsafe fn follow(buf: &'a [u8], loc: usize) -> Self::Inner {";
    if (has_encrypted) {
      code_ += "    Self { _tab: unsafe { ::flatbuffers::Table::new(buf, loc) }, encryption_ctx: None }";
    } else {
    ...
    code_ +=
        ...
        "Self {";
    if (has_encrypted) {
      code_ += "    {{STRUCT_TY}} { _tab: table, encryption_ctx: None }";
    } else {
      code_ += "    {{STRUCT_TY}} { _tab: table }";
    }
    code_ += "  }";

    // Add overloaded init with encryption context if needed
    if (has_encrypted) {
      code_ += "  #[inline]";
      code_ +=
          "  pub unsafe fn init_from_table_with_ctx(table: ::flatbuffers::Table<'a>, ctx: Option<Vec<u8>>) -> "
          "Self {";
      code_ += "    {{STRUCT_TY}} { _tab: table, encryption_ctx: ctx }";
      code_ += "  }";
    }
  ...
  void GenStruct(const StructDef& struct_def) {
    ...
    ForAllStructFields(struct_def, [&](const FieldDef& field) {
      ...
      if (IsStruct(field.value.type)) {
        ...
        // Add encryption support for fields with (encrypted) attribute
        if (field.attributes.Lookup("encrypted") != nullptr) {
          code_ += "  crate::flatbuffers_encryption::decrypt_scalar(raw_value, &self.encryption_ctx, {{FIELD_OFFSET}})";
        }
      }
      code_ += "}\n";
      // Setter.
